...
| `FINAL_FIGURE_FORMATS` | svg, pdf, tiff, png | Journal-submission copies written to `results/final_figures/` |

### The SVG and PDF copies

Both come from the same source: the true-vector SVG that
`assemble_manuscript_figure_svg()` writes next to each assembled PNG, with its
artboard set to the journal column width (89 mm single-column, 180 mm for
Figure 6). The PDF is that SVG converted by an external vector converter, so
the two can never disagree and the PDF page size *is* the column width.

`06_submission/reviewer_comments/final_figures_combined.pdf` in the master repo
is assembled from these PDFs, one figure per page.

**The PDF branch needs a vector SVG→PDF converter on `PATH`** — Inkscape
(found automatically at `/Applications/Inkscape.app/…` on macOS),
`rsvg-convert`, or `cairosvg`, tried in that order by
`figure_assembly.svg_to_pdf()`. If none is installed the run **raises**; it does
not fall back on wrapping the PNG, because such a PDF looks identical in a
viewer while being a raster at the journal's submission step. Drop `"pdf"` from
`FINAL_FIGURE_FORMATS` to skip it on a machine without a converter.

Figure 3 and Table 1 are PIL-assembled rasters with no vector source, so their
`.svg`/`.pdf` carry an embedded image rather than paths. Every other figure is
vector end to end.
...
6. **Figures 4–7 are not bit-reproducible between runs.** Three plotting calls
   draw from the global NumPy RNG without a seed: the `jitter=0.2` strip plots
   in `multipanel_boxplot_grid.py`, the `add_x_jitter` noise in
   `correlation_matrix_plot.py`, and the bootstrap CI band `sns.regplot` draws
   in the same file. Every figure carrying a boxplot or correlation panel
   therefore differs cosmetically from run to run (≈1–3 % of pixels, measured);
   Figures 2, 3 and Table 1 have none of those panels and are reproducible.
   Only the point positions and the CI band move — no estimate, interval or

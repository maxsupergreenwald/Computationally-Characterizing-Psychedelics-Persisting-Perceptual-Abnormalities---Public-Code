...
# asks for; "pdf" is that same vector master as a page-sized document, and is
# what 06_submission/reviewer_comments/final_figures_combined.pdf is built from;
# "tiff" is the raster fallback some journals require; "png" is for quick viewing
# and for pasting into drafts.  Also accepts "jpeg".
#
# "pdf" is converted from the journal-width SVG by an external vector converter
# (Inkscape / rsvg-convert / cairosvg — see figure_assembly.svg_to_pdf), so it is
# fully vector and its page size is the journal column width.  If none of those
# is installed the run raises rather than writing a raster-wrapped PDF; drop
# "pdf" from this tuple to skip it on a machine without a converter.
FINAL_FIGURE_FORMATS = ("svg", "pdf", "tiff", "png")
...
if RUN_LINKED_FIGURES:
    ...
    from figure_assembly import scale_svg_to_dpi as _scale_svg_to_dpi
    from figure_assembly import svg_to_pdf as _svg_to_pdf
    ...
    def _build_final_svg(stem, src_path, dest_svg):
        """Write the journal-width SVG master for one figure to *dest_svg*.

        Prefers the true-vector SVG that assemble_manuscript_figure_svg() wrote
        alongside the PNG; falls back to wrapping the PNG only when no such
        sibling exists (Figure 3 and Table 1, which are PIL-assembled rasters).
        The artboard is then set to the journal column width, which is also what
        fixes the page size of the PDF derived from it.
        """
        vector_svg = src_path.with_suffix('.svg')
        if vector_svg.exists():
            shutil.copyfile(vector_svg, dest_svg)
        else:
            _raster_to_svg(str(src_path), str(dest_svg), dpi=FIGURE_DPI)
        _w_mm = 180 if stem in _SVG_DOUBLE_FIGS else _SVG_WIDTH_MM
        _scale_svg_to_dpi(dest_svg, target_width_mm=_w_mm, top_margin_mm=5)
        return dest_svg
    ...
    for stem, src in manuscript_outputs.items():
        ...
        # The SVG master is built once per figure and both the .svg and .pdf
        # outputs come from it, so the two can never disagree and the loop does
        # not depend on the order of FINAL_FIGURE_FORMATS.  It goes to a scratch
        # file when "svg" is not a requested output format.
        _svg_scratch_dir = None
        _svg_master = None
        if {"svg", "pdf"} & set(FINAL_FIGURE_FORMATS):
            if "svg" in FINAL_FIGURE_FORMATS:
                _svg_master = _out_dir / f"{stem}.svg"
            else:
                _svg_scratch_dir = _tempfile.mkdtemp(prefix="final_fig_svg_")
                _svg_master = Path(_svg_scratch_dir) / f"{stem}.svg"
            _build_final_svg(stem, src_path, _svg_master)
        for _fmt in FINAL_FIGURE_FORMATS:
            dest = _out_dir / f"{stem}.{_fmt}"
            if _fmt == "svg":
                pass          # already written above as the SVG master
            elif _fmt == "pdf":
                # Vector all the way through: the SVG master's marks and its
                # journal-width artboard become the PDF's paths and page size.
                # Raises if no vector converter is installed rather than
                # silently emitting a raster-wrapped PDF.
                _svg_to_pdf(_svg_master, dest)
        ...
        if _svg_scratch_dir is not None:
            shutil.rmtree(_svg_scratch_dir, ignore_errors=True)

...
"""
mediation_results_table_nominal_sensitivity.py
===============================================
Publication-style table summarising mediation analysis results for the two
HGF nominal-model parameters (vch_nu, vch_beta) under the manuscript's primary
covariate specification (nice_covariates_spusers).

This is a subset companion to mediation_results_table.py, restricted to 2
mediators instead of 4.
...
Mediators (2 per panel):  vchbeta, vchnu

Column structure (same for both panels):
  Path | beta | beta 94% HDI | P(beta!=0) | Delta | Delta 94% HDI | P(Delta!=0)
...
Data sources (per model directory results/{dv}/mediation_models/{dv}_{spvar}_{med}_{cov}/)
  path_coefficients_summary.csv   -> beta, hdi_lower_94, hdi_upper_94, prob
  path_counterfactual_summary.csv -> Delta, hdi_low, hdi_high, p
  mc_mediation_summary.csv        -> Delta, hdi_low, hdi_high, p (NIE/NDE/TE/PMed)
...
Output
------
  results/supplement/tables/supplementary_table_s3cd.png
  results/supplement/tables/supplementary_table_s3cd.csv
  results/supplement/tables/supplementary_table_s3cd.docx

Usage
-----
  cd 04_visualizations
  /usr/local/bin/python3.12 supplement/mediation_results_table_nominal_sensitivity.py
"""
...
OUTPUT_FILE  = OUTPUT_DIR / "supplementary_table_s3cd.png"
OUTPUT_CSV   = OUTPUT_DIR / "supplementary_table_s3cd.csv"
OUTPUT_DOCX  = OUTPUT_DIR / "supplementary_table_s3cd.docx"
...
# Only the two HGF nominal-model parameters (nominal likelihood)
MEDIATORS = ["vchbetanominal", "vchnunominal"]

MEDIATOR_LABELS = {
    "vchbetanominal": "VCH Decision Noise (\u03b2)",       # beta
    "vchnunominal":   "VCH Top-down Bias (\u03bd)",         # nu
}

# Column header strings
BETA_COLS  = ["\u03b2", "\u03b2 94% HDI", "P(\u03b2\u22600)"]   # beta, !=
...
DELTA_COLS = [DELTA_EST_HEADER, "\u0394 94% HDI", "P(\u0394\u22600)"]    # Delta, !=
...
def get_path_coef(df: pd.DataFrame, path: str) -> dict:
    """Extract one path's beta, 94% HDI, and max-direction P from path_coefficients_summary."""
    row = df[df["path"] == path]
    if len(row) == 0:
...
def get_cf_effect(df: pd.DataFrame, effect_prefix: str, *,
    ...
    """Extract one effect's Delta, HDI, and max-direction P from counterfactual/MC CSV.
...
def _make_data_row(label: str, beta: dict, delta: dict) -> dict:
    """Build one display row given pre-fetched beta and Delta result dicts."""
    cells = [
...
def build_rows(dv: str, spvar: str, has_hu: bool) -> list[dict]:
    ...
    for mediator in MEDIATORS:
        ...
        # MC mediation effects (Delta only; no normalised beta)
        mc_specs = [
            ("NIE",    "NIE"),
            ("NDE",    "NDE"),
            ("Total",  "TE"),
...
def main() -> None:
    ...
    # Approximate panel heights (inches)
    COL_H = 0.30
    SEC_H = 0.26
    DAT_H = 0.21
    GAP_H = 0.40

...
"""
regression_results_table_nominal_sensitivity.py
================================================
Bayesian regression table for vch_nu and vch_beta as predictors of
hppd_binary and caps_vision, using the nice_covariates_spusers covariate set.

This is a subset companion to regression_results_table.py, restricted to the
two HGF nominal-model parameters (nu, beta) under the manuscript's primary
covariate specification.

Panels
------
  a -- hppd_binary (PPA History):
    Predictor | beta | beta 94% HDI | P(beta!=0) | Delta | Delta 94% HDI | P(Delta!=0) | N

  b -- caps_vision (CAPS Vision Score; adds hurdle component):
    Predictor | beta | beta 94% HDI | P(beta!=0) | hu beta | hu beta 94% HDI | P(hu_beta!=0) | Delta | Delta 94% HDI | P(Delta!=0) | N

Data sources
  beta, HDI94, P  -> results/sensitivity_analyses_single_paths/existingresults_manuscript.csv
  Delta, P        -> results/sensitivity_analyses_single_paths/existingresults_manuscript_counterfactual.csv
  Covariate set: nice_covariates_spusers

Output
  results/supplement/tables/supplementary_table_s3ab.png
  results/supplement/tables/supplementary_table_s3ab.csv
  results/supplement/tables/supplementary_table_s3ab.docx

Usage
  cd 04_visualizations
  /usr/local/bin/python3.12 supplement/regression_results_table_nominal_sensitivity.py
"""
...
MANUSCRIPT_CSV     = RESULTS_BASE / "sensitivity_analyses_single_paths" / "existingresults_manuscript.csv"
...
OUTPUT_FILE        = OUTPUT_DIR / "supplementary_table_s3ab.png"
OUTPUT_CSV         = OUTPUT_DIR / "supplementary_table_s3ab.csv"
OUTPUT_DOCX        = OUTPUT_DIR / "supplementary_table_s3ab.docx"

# Only the two HGF nominal-model parameters
PREDICTORS    = ["vch_nu_nominal_normalized", "vch_beta_nominal_normalized"]
...
# Column header strings (same as regression_results_table.py)
...
def load_data() -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    sens = pd.read_csv(MANUSCRIPT_CSV)
...
def build_rows_panel_a(sens: pd.DataFrame, cf: pd.DataFrame) -> list[dict]:
    """Panel a -- hppd_binary. No section headers, just data rows."""
    DV = "hppd_binary"
    rows: list[dict] = []
    for pred in PREDICTORS:
        b = lookup_beta(sens, pred, DV)
        d = lookup_delta(cf, pred, DV)
...
def build_rows_panel_b(sens: pd.DataFrame, cf: pd.DataFrame) -> list[dict]:
    """Panel b -- caps_vision. No section headers, includes hu columns."""
    DV = "caps_vision"
    rows: list[dict] = []
    for pred in PREDICTORS:
        hu_var = "hu_" + pred
...
def draw_panel(
    ...
    """Render one panel table on *ax*.

    Style mirrors regression_results_table.py / generate_publication_table.py.
    """
    ax.axis("off")
...
def main() -> None:
    ...
    # Figure sizing -- 2 predictors, no section headers
    n_preds = len(PREDICTORS)   # 2

    # Row heights in inches: column header 0.38, data 0.25
    panel_h = 0.38 + n_preds * 0.25
    GAP_H   = 0.45
    total_h = 2 * panel_h + GAP_H

    # Width driven by panel b: 3 (beta) + 3 (hu) + 3 (Delta) + 1 (N) = 10 data cols
    n_max_data = len(BETA_COLS) + len(HU_COLS) + len(DELTA_COLS) + 1   # 10
    fig_w = 2.8 + n_max_data * 1.35

    print(f"Figure: {fig_w:.1f} x {total_h:.1f} in")

...
# ---------------------------------------------------------------------------
# SVG → PDF conversion (vector-preserving)
# ---------------------------------------------------------------------------

# Converters are tried in this order.  Each entry is (label, argv_builder).
# All three preserve vector geometry — nothing here rasterizes the figure.
#
# Inkscape is listed first because it is the only one of the three that is
# actually installed on the analysis machine, and it handles the svgutils-
# assembled artboards (nested <svg>/<g> transforms, mm units) correctly.
# rsvg-convert and cairosvg are the same cairo backend reached by other means
# and are here so the pipeline still works on a machine without Inkscape.
#
# There is deliberately NO raster fallback.  A PDF produced by re-wrapping the
# PNG would look identical in a viewer while silently being a raster image at
# the journal's submission step, so an unavailable converter is raised as an
# error rather than worked around.
_SVG_TO_PDF_CONVERTERS = (
    ('inkscape', lambda exe, svg, pdf: [
        exe, str(svg),
        '--export-type=pdf',
        f'--export-filename={pdf}',
        # Export the artboard as set by scale_svg_to_dpi(), not the content
        # bounding box, so the PDF page is exactly the journal column width.
        '--export-area-page',
    ]),
    ('rsvg-convert', lambda exe, svg, pdf: [
        exe, '--format=pdf', '--output', str(pdf), str(svg),
    ]),
    ('cairosvg', lambda exe, svg, pdf: [
        exe, str(svg), '-f', 'pdf', '-o', str(pdf),
    ]),
)

# Inkscape on macOS installs as an .app bundle whose CLI is not on PATH.
_INKSCAPE_FALLBACK_PATHS = (
...
def _find_svg_to_pdf_converter():
    """Return (label, executable_path, argv_builder) for the first converter found.

    Returns None if no vector SVG → PDF converter is installed.
    """
    import shutil as _shutil

    for label, argv_builder in _SVG_TO_PDF_CONVERTERS:
        exe = _shutil.which(label)
        if exe is None and label == 'inkscape':
            for candidate in _INKSCAPE_FALLBACK_PATHS:
                if os.path.isfile(candidate) and os.access(candidate, os.X_OK):
                    exe = candidate
                    break
        if exe:
            return label, exe, argv_builder
    return None


def svg_to_pdf(svg_path, pdf_path):
    """Convert a vector SVG to a fully vector PDF.

    Every mark in the output is a PDF path or an embedded-font glyph — no
    resampling, no raster re-encode, no lossy compression.  (Stream contents are
    Flate-compressed by the converter, which is lossless and is what every
    journal PDF uses; that is not a rasterization step.)

    Whatever is raster in the *source* SVG stays raster: `raster_to_svg()`
    wraps a PNG in an <image> element, and a PDF made from such an SVG carries
    that same image.  Only figures assembled by `assemble_manuscript_figure_svg()`
    are vector end to end.

    Parameters
    ----------
    svg_path : str or Path
        Source SVG.  Its artboard (width/height in mm) becomes the PDF page size,
        so call `scale_svg_to_dpi()` first if a specific column width is wanted.
    ...
    ------
    RuntimeError
        If no vector converter is installed, or if the converter fails.  This is
        deliberate — see the comment on `_SVG_TO_PDF_CONVERTERS`.
    """
    import subprocess

    found = _find_svg_to_pdf_converter()
    if found is None:
        raise RuntimeError(
            "No vector SVG->PDF converter found. Install one of: Inkscape "
            "(https://inkscape.org), librsvg's rsvg-convert, or cairosvg "
            "(pip install cairosvg). Refusing to fall back on a raster PDF."
        )
    label, exe, argv_builder = found

    Path(pdf_path).parent.mkdir(parents=True, exist_ok=True)
    argv = argv_builder(exe, Path(svg_path), Path(pdf_path))
    proc = subprocess.run(argv, capture_output=True, text=True)
    if proc.returncode != 0 or not Path(pdf_path).is_file():
        raise RuntimeError(
            f"{label} failed to convert {svg_path} -> {pdf_path}\n"
            f"  command: {' '.join(argv)}\n"

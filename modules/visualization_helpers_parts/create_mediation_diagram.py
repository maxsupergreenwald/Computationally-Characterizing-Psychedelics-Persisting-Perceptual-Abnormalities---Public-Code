...
def create_mediation_diagram(
    ...
    # When show_indirect_pd=True, display this value (response-scale Δ_med) in the
    # centre label instead of the generic "Δ_med = change" legend.  Should be
    # sourced from mc_mediation_summary.csv → median for the NIE row (hence the
    # "med" subscript — see the rendering note further down).
    nie_delta=None,
    ...
    try:
        ...
        # In counterfactual diagrams, show the NIE Δ_med above the P(Δ≠0) text.
        ...
        # generic legend "Δ_med = change".
        #
        # The subscript is matplotlib mathtext ("$_{med}$"), the same rendering the
        # supplementary mediation tables use for this token (DELTA_EST_HEADER in
        # 04_visualizations/supplement/mediation_results_table.py).  It marks the
        # quantity as the posterior MEDIAN of the MC-integrated indirect effect —
        # a different posterior summary from the means carried by the a/b/c' paths
        # on the same diagram, which keep the plain coef_label "Δ".  P(Δ≠0) below
        # also keeps the plain Δ: it is quantile-based and so is unaffected by
        # which point summary is displayed.  This matches the prose convention in
        # 05_results_narrative/results_narrative.py.
        if show_indirect_pd:
            if nie_delta is not None:
                if nie_delta_as_pct:
                    nie_label = f"\u0394$_{{med}}$ = {nie_delta * 100:+.1f}%"
                else:
                    nie_label = f"\u0394$_{{med}}$ = {nie_delta:+.3f}"
            else:
                nie_label = "\u0394$_{med}$ = change"
            ax.text(
                center_x,
